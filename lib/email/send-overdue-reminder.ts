...
export async function sendOverdueReminderEmail(invoice: Invoice): Promise<SendOverdueReminderResult> {
  ...
  const dueDateText = invoice.due_date
    ? `<p style="margin:0 0 8px;color:#dc2626;font-size:14px;">Was due: <strong>${new Date(invoice.due_date).toLocaleDateString("en-US", { month: "long", day: "numeric", year: "numeric" })}</strong></p>`
    : ""
...
<body style="font-family:-apple-system,BlinkMacSystemFont,'Segoe UI',sans-serif;background:#f9fafb;margin:0;padding:32px 16px;">
  <div style="max-width:600px;margin:0 auto;background:#fff;border-radius:12px;overflow:hidden;border:1px solid #e5e7eb;">
    ...
    <div style="padding:32px;">
      <div style="background:#fef2f2;border:1px solid #fecaca;border-radius:8px;padding:12px 16px;margin-bottom:24px;">
        <p style="margin:0;font-size:14px;font-weight:600;color:#dc2626;">Payment Overdue</p>
      </div>
      ...
      <p style="margin:0 0 24px;color:#374151;font-size:14px;">Your invoice is now past due. Please arrange payment at your earliest convenience to avoid any service disruptions.</p>
      ${dueDateText}
      ...
      <div style="display:flex;justify-content:flex-end;margin-bottom:24px;">
        <table style="font-size:14px;min-width:240px;">
          ...
          <tr style="border-top:1px solid #e5e7eb;">
            ...
            <td style="padding:8px 0 4px;text-align:right;font-weight:700;color:#dc2626;font-size:16px;">$${Number(invoice.total).toFixed(2)}</td>
          </tr>
        </table>
      </div>
      ...
      <p style="margin:0;font-size:13px;color:#6b7280;">If you've already sent payment, please disregard this notice. Thank you for your business.</p>
    </div>
  </div>
</body>
</html>`
  ...
  const { error } = await resend.emails.send({
    ...
    subject: `Payment Overdue: Invoice ${invoice.invoice_number} — $${Number(invoice.total).toFixed(2)} past due`,
    html,
  })

  if (error) return { success: false, message: error.message }
  return { success: true, message: `Overdue reminder sent to ${invoice.client.email}.` }
}
