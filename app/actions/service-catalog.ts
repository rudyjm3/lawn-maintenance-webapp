...
export async function saveRecurrenceRule(
  ...
  if (id) {
    ...
    // Remove stale scheduled jobs from before today that were generated by the old rule.
    // Must delete route_stops first to avoid FK violation.
    const { data: staleJobs } = await db
      .from("jobs")
      .select("id")
      .eq("property_service_id", fields.property_service_id)
      .eq("status", "scheduled")
      .lt("service_date", todayUtc())
    if (staleJobs && staleJobs.length > 0) {
      const staleIds = staleJobs.map((j: { id: string }) => j.id)
      await db.from("route_stops").delete().in("job_id", staleIds)
      const { error: cleanupError } = await db.from("jobs").delete().in("id", staleIds)
  ...
  // Remove any stale past scheduled jobs (e.g. from a previous rule on the same property service).
  // Must delete route_stops first to avoid FK violation.
  const { data: staleJobs } = await db
    .from("jobs")
    .select("id")
